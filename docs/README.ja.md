...
の名前を付けてエンドポイントを保持しています。起動する MCP サーバも、読み込むスキル
も、事情はまったく同じです。プロバイダを一つ追加する、あるいは二つの間で切り替える
だけで、三つのファイルを手で開くことになります。ConfAI はそれを一つのコマンドで行
い、変更していない箇所を整形し直すことは決してありません。
...
- `confai provider sync` は、エンドポイントが実際に提供しているモデル一覧を、コンテ
  キスト長と出力上限つきで書き込みます。
- `confai mcp list` と `confai skill list` は、エージェントが起動する MCP サーバと
  読み込むスキルについて同じことをします。
...
| `mcp list` · `mcp doctor` | 各エージェントの MCP サーバ · それぞれ起動できるか |
| `mcp add <name>` · `mcp remove <name>` | サーバの追加・変更 · 削除 |
| `mcp toggle <name>` | エージェントが対応していれば、削除せずにサーバを止める |
| `mcp preset list` · `mcp preset apply <id>` | 既製のサーバ定義と、その適用 |
| `skill list` · `skill path` | 各エージェントのスキル · その保存場所 |
| `skill doctor` | エージェントが黙って無視するスキルと、その理由 |
| `skill copy <name>` · `skill remove <name>` | スキルを別のエージェントへコピー · 削除 |
...
`list`、`doctor`、`about`、`update`、`preset list`、`mcp preset list` はエージェン
ト指定を取りません。常にすべてを対象にします。`skill copy` と `skill remove` は独自
の指定を取ります。前者は `--from` / `--to`、後者は必須の `--agent` です。

</details>

<details>
<summary><b>MCP サーバ</b> — 同じ一覧、三者三様の設定の形</summary>

エージェントはそれぞれ独自の MCP サーバ群を起動しますが、三つとも記録の仕方も場所も
違います。Codex は TOML の `mcp_servers` に、コマンドと独立した `args` のリストとい
う形で保持します。Claude Code は `~/.claude.json` の `mcpServers` に保持します。こ
れは `settings.json` ではない三つ目のファイルです。opencode は設定の `mcp` に保持し
ますが、そこではコマンドはプログラムと引数ではなく一つのリストであり、環境変数のブ
ロックは `env` ではなく `environment` と呼ばれ、サーバは削除せずに無効にできます。
...
`mcp add` は、stdio サーバなら `--command` と、順序を保つ繰り返し指定可能な `--arg`
を、リモートサーバなら `--url` を取り、さらに繰り返し指定できる `--env KEY=VALUE`
を取ります。`mcp doctor` は秒単位の `--timeout`（既定 10）を取ります。
`mcp preset apply` は、プリセット id 以外の名前で登録するための `--name` を取りま
す。

**`confai mcp doctor` は何も起動しません。** stdio サーバについては実行ファイルを
`PATH` 上で解決し、リモートサーバについてはそのエンドポイントを呼びます。設定に書か
れた任意のコマンドを実行して様子を見るのは診断ではなく、設定にあるものをただ実行し
ているだけです。`npx` のようなランチャーはランチャーとしてそのまま報告されます。背
後のパッケージは取得しない限り検証できないからです。

`~/.claude.json` には進行中のセッション状態が入っていて、Claude Code が絶えず書き込
んでいます。そのため ConfAI は、MCP の編集が実際に何かを変えたときにだけこのファイ
ルを書き直します。理由もなくエージェントと取り合うことはしません。

`mcp toggle` は、エージェントにその状態を書いておく場所がある場合に働きます。現時点
では opencode です。Codex と Claude Code には無効化のフラグがありません。ConfAI は
それをはっきり伝え、代わりにサーバを削除するよう促します。できたふりはしません。

組み込みの定義 9 個は [`presets/mcp/`](../presets/mcp/) にあります: continuum、
context7、playwright、github、git、fetch、filesystem、memory、
sequential-thinking。自分用のものは `~/.confai/presets/mcp/` に置きます。

</details>

<details>
<summary><b>スキル</b> — 各エージェントが持つものと、黙って無視しているもの</summary>

スキルとは `SKILL.md` を含むディレクトリで、エージェントはディレクトリを走査して見
つけます。Claude Code と opencode はどちらもこの方式で、設定ファイルの隣の `skills/`
を使います。Codex にはスキルという仕組み自体がありません。プラグインは別物であり、
ConfAI はそのことをそのまま伝えます。存在しないディレクトリをでっち上げたりはしませ
ん。
...
`skill copy` には `--from` が必要です。`--to` を省くと、スキルを持てる他のすべての
エージェントにインストールされます。`--force` は、コピー先に同名のスキルがすでにあ
る場合にそれを置き換えます。同じスキルが複数のエージェントで役に立つのに、共有の置
き場所がないので、このコマンドがあります。

`skill doctor` は、スキルが結局どこからも読み込まれず、しかもそのことが何も告げられ
ないパターンを報告します。読み取れる `SKILL.md` がないディレクトリ、エージェントが
使いどころを判断するための `description` がない front matter、あるいは front matter
の `name` がディレクトリ名と食い違っている場合です。エージェントはスキルをディレク
トリ名で参照します。

`skill remove` は `--agent` が必須で、ディレクトリを削除します。設定ファイルのよう
なバックアップはディレクトリには存在しないため、**`confai undo` では戻せません。**
削除の前に、これから消すパスを表示します。
...
`v` は右ペインを三つの表示のあいだで順に切り替えます。プロバイダ → MCP サーバ →
スキル → プロバイダ、の順です。そのエージェントが持たない表示は飛ばされるので、
Codex ではプロバイダと MCP サーバのあいだを行き来するだけになります。Codex はスキル
を持たないからです。
...
| `↑` `↓` / `k` `j` | 移動 · `Tab` `←` `→` ペイン切り替え |
| `v` | 右ペインの表示を順に切り替える |
| `/` または `Ctrl+F` | id・ホスト・モデルで一覧を絞り込み |
| `m` | このエージェントが使うモデルを選ぶ |
| `s` / `S` | モデルを同期 · 同期して古いものを削除 |
| `?` | このツールについてと、キー割り当ての全一覧 |
| `r` `q` | ディスクから再読み込み · 終了 |

残りは、右ペインがそのとき表示しているものに対して働きます:

| キー | プロバイダ | MCP サーバ | スキル |
|---|---|---|---|
| `Enter` | 詳細 | 詳細 | 詳細 |
| `u` | エージェントをこれ経由にする | 有効・無効を切り替え | — |
| `a` | 追加 | 追加 | — |
| `e` | 編集 | 編集 | — |
| `d` | 削除 | 削除 | 確認のうえ削除 |
| `c` / `C` | 確認 · すべて確認 | 確認 · すべて確認 | — |
| `p` | プリセットを適用 | 既製のサーバを追加 | — |
| `y` | — | — | 別のエージェントへコピー |

**スキルには追加も編集もありません。** スキルは一覧画面で書くものではなく、それはテ
キストエディタの仕事です。スキル表示ではこれらのキーは何もしません。エラーを出すこ
ともしません。

**スキルの削除は、このプログラムで唯一取り消せない操作です。** ほかの削除はすべて設
定ファイルの書き換えであり、事前に取ったバックアップから `confai undo` で戻せます。
スキルはディレクトリであり、ディレクトリのバックアップはありません。確認のメッセー
ジは、何かを削除する前にそのことをはっきり伝えます。
...
MCP サーバとスキルは、これまた別の場所にあります:

| エージェント | MCP サーバ | 無効化 | スキル |
|---|---|---|---|
| Codex | `config.toml` の `mcp_servers` | 不可 | なし — プラグインは別の仕組み |
| Claude Code | `~/.claude.json` の `mcpServers` | 不可 | `~/.claude/skills/` |
| opencode | `opencode.json` の `mcp` | 可 | 設定ファイルの隣の `skills/` |
